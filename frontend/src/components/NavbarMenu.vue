...
function updateFocusableElements() {
  ...
  const elements = menuRef.value.querySelectorAll<HTMLElement>(
    'a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])'
  )
  focusableElementsRef.value = Array.from(elements)
}
...
function handleKeydown(e: KeyboardEvent) {
  ...
  if (e.key === 'Tab') {
    const elements = focusableElementsRef.value
  ...
  if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
    const elements = focusableElementsRef.value.filter(el => el.tagName === 'A' || el.tagName === 'BUTTON')
    if (elements.length === 0) return
...
onMounted(() => {
  ...
  setTimeout(() => {
    const firstLink = menuRef.value?.querySelector('a, button') as HTMLElement
    firstLink?.focus()
  }, 0)
})
...
<template>
  <div
    ref="menuRef"
    class="fixed right-4 top-16 z-50 w-72 max-w-[calc(100vw-2rem)]"
    role="menu"
    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
      ...
      <div class="border-b border-slate-200 px-4 py-3">
        <div class="flex items-center gap-3">
          <div class="h-10 w-10 shrink-0 rounded-full bg-teal-100 flex items-center justify-center">
            <img
      ...
      <!-- Navigation Groups -->
      <nav class="max-h-[50vh] overflow-y-auto px-2 py-2">
        <div v-for="group in navGroups" :key="group.label">
          <p class="px-3 py-1.5 text-xs font-semibold uppercase tracking-wider text-slate-400">
            {{ group.label }}
          </p>
          <div class="space-y-0.5 pb-3">
            <router-link
          ...
          <hr v-if="group !== navGroups[navGroups.length - 1]" class="border-slate-100 my-2" />
        </div>
      </nav>
