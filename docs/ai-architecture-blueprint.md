...
## 6.4 Health Intelligence Layer (Ask MedExplain)

`POST /api/v1/health/query` answers a natural-language question about the user's
own data. It is deliberately split: deterministic reasoning and ownership stay in
Laravel; explanation happens in FastAPI. The LLM never queries the database and
never performs deterministic math.

### Intent registry (Laravel, deterministic)

`IntentRegistry` maps a question to exactly one `HealthQueryIntent` via regex
rules — no LLM routing. Intents:

`REPORT_COMPARISON`, `LAB_TREND`, `MEDICATION_CONTEXT`, `RECENT_HEALTH_CHANGES`,
`CURRENT_VS_PREVIOUS`, `HEALTH_TIMELINE`, `LAB_HISTORY`,
`MEDICATION_HISTORY`, `GENERAL_HEALTH_QUESTION`.

`requires_rag` is true only for `REPORT_COMPARISON`, `CURRENT_VS_PREVIOUS`, and
`GENERAL_HEALTH_QUESTION` (those need medical context to explain what the values
*mean*). Trend/history intents are anchored on deterministic numbers only.

### Deterministic services (Laravel)

- `ReportComparisonService` — new/changed/removed/unchanged tests, delta, percent,
  direction, status/unit change.
- `LabTrendEngine` — chronological series, net and consecutive deltas, units,
  date range.
- `MedicationAtDateResolver` — active / ended / not-started / unknown-timing
  against a result date (open-ended, same-day, and overlap handling).
- `RecentHealthChangesService` — aggregated, sorted changes including
  medication additions/endings.

`HealthContextService` retrieves only the sections the detected intent needs
(lazy, user-scoped) so the payload stays small and the full history is never
sent to the AI service.

### FastAPI health-query layer

- Request/response validated with Pydantic at the boundary.
- Retrieval is gated: always for general questions, plus any test names present
  in the structured sections. Grounding is the only source of reference ranges.
- `LLMGateway.generate_json(task="health_query", response_schema=HealthQueryResponse)`
  with bounded re-request on schema-validation failure.
- Safety gates applied to every answer:
  - Urgent-hint questions are deferred to emergency care without calling the LLM.
  - Missing required data returns a deterministic "unavailable" answer.
  - `sources` come only from the curated store; `data_used` is echoed from the
    request (never invented by the model).
  - Facts/changes referencing unknown tests or numbers not present in the data
    are dropped.
  - Empty output falls back to a deterministic summary computed from the data.

### Observability

The gateway already emits `GatewayEvent` (provider, model, task, latency, usage,
error — never content). The health-query flow reuses it with `task="health_query"`;
`query_id` is generated by Laravel and passed in the request body so support can
correlate an incident to a query without logging the user's health context.
...
### Phase 6 — Health Intelligence Layer (Ask MedExplain)

- **Files:** backend `HealthQueryService` + `IntentRegistry` + deterministic
  services; FastAPI `health_query` schemas + `HealthIntelligenceService`; frontend
  `/ask` view.
- **Work:** intent registry, deterministic comparison/trend/medication/changes
  engines, structured prompt with strict schema, safety gates, sectioned UI.
- **Verify:** intent-detection tests; deterministic-service tests; FastAPI
  safety-gate tests; end-to-end scenarios across all three suites.
